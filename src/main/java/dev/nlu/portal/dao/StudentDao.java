...
public class StudentDao extends BaseDAO implements DAO<Student> {
    ...
    public List<Student> findAll() {
        ...
        String sql = "SELECT s.*, u.username, u.role, u.enabled FROM students s " + "JOIN users u ON s.user_id = u.id";
    ...
    public boolean save(Student t, Connection conn) {
        String sql = "INSERT INTO students (user_id, student_code, full_name, birthday, gender, " + "status, phone_number, identity_card, ethnicity, religion, birth_place, address) " + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public boolean update(Student t, Connection conn) {
        String sql = "UPDATE students SET full_name = ?, birthday = ?, gender = ?, status = ?, " + "phone_number = ?, identity_card = ?, address = ? WHERE id = ?";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public User mapResultSetToUser(ResultSet rs) throws SQLException {
        return User.builder().id(rs.getLong("user_id")).username(rs.getString("username")).passwordHashed(rs.getString("password_hash")).enabled(rs.getBoolean("enabled")).role(Role.valueOf(rs.getString("role"))).build();
    }

    private Student mapResultSetToStudent(ResultSet rs) throws SQLException {
        User user = mapResultSetToUser(rs);

        return Student.builder().id(rs.getLong("id")).user(user).studentCode(rs.getString("student_code")).fullName(rs.getString("full_name")).birthday(rs.getDate("birthday") != null ? rs.getDate("birthday").toLocalDate() : null).gender(rs.getString("gender")).status(rs.getString("status")).phoneNumber(rs.getString("phone_number")).identityCard(rs.getString("identity_card")).ethnicity(rs.getString("ethnicity")).religion(rs.getString("religion")).birthPlace(rs.getString("birth_place")).nationality(rs.getString("nationality")).emailPersonal(rs.getString("email_personal")).address(rs.getString("address")).build();
    }
